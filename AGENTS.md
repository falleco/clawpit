...
7. [ ] Tested on at least one platform
8. [ ] User-facing strings are clear and helpful
9. [ ] **Path handling works on both Windows (WSL) and Unix**
10. [ ] **Docker Compose commands tested with v2**
11. [ ] **Git operations tested through platform abstraction**
12. [ ] **Windows testing includes WSL2 integration** (if applicable)
...
```bash
# 1. Verify frontend build (TypeScript + Vite)
bun run build

# 2. Verify Rust code compiles (quick check)
cd src-tauri && cargo check

# 3. Verify full Tauri build (Rust + Frontend) - for release
bun run tauri build
```

**Rules:**
- Never mark a task as complete if either build fails
...
1. `bun run build` succeeds (frontend)
2. `cargo check` reports only system library errors, not code errors
