// Linux platform implementation - native command execution
...
/// Common paths where Docker might be installed on Linux
...
/// Common paths where Git might be installed on Linux
const GIT_PATHS: &[&str] = &[
    "/usr/bin/git",
    "/usr/local/bin/git",
];

/// Linux command executor - runs commands natively
...
impl LinuxExecutor {
    ...
    fn get_docker_env() -> Vec<(String, String)> {
        ...
        // Get DOCKER_HOST if set (important for remote Docker or custom socket locations)
    ...
    /// Execute a command natively with proper PATH and environment
    ...
    fn get_docker_path(&self) -> Result<&str, String> {
        self.docker_path
            .as_deref()
            .ok_or_else(|| {
                "Docker not found. Please install Docker: https://docs.docker.com/engine/install/".to_string()
            })
    }
    ...
    fn get_git_path(&self) -> Result<&str, String> {
        self.git_path
            .as_deref()
            .ok_or_else(|| {
                "Git not found. Please install Git using your package manager (e.g., sudo apt install git)".to_string()
            })
    }
}

#[async_trait]
impl CommandExecutor for LinuxExecutor {
    ...
    async fn run_docker_compose_with_tty(
        ...
        // we need to insert -T (disable pseudo-tty) for non-interactive execution
    ...
    fn normalize_path(&self, path: &str) -> String {
        // Linux paths are already in the correct format
        path.to_string()
    }
}
